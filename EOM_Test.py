...
def get_sweep_dispersion(sweep_cavity_indices, sweep_eom_indices, sweep_cavity, ax):
    ...
    while n < num_of_FSRs:
        ...
        result = calculate_FSR(starting_index,nextindex,sweep_cavity_indices, sweep_eom_indices, sweep_cavity, ax)
        FSRs.append(result[0])
        v_scans.append(result[1])
        n+=1
    return FSRs, v_scans
...
def calculate_FSR(startindex, nextindex,sweep_cavity_indices, sweep_eom_indices,sweep_cavity,ax):
    ...
    v_scan = (f_mod1 - m*FSR)/t_mod1
    #here, the average is done over 23 FSRs
    return FSR, v_scan
...
#%% Fit first dispersion curve to see accuracy
def poly2(x,p):
    return p['b']*x + p['c']
...
x = np.arange(0,len(FSRs1[0]),1)
minner = Minimizer(fcn2min,params, fcn_args = (np.arange(0,len(FSRs1[0]),1),FSRs1[0]))
result = minner.minimize()
report_fit(result)
report = fit_report(result)
fitted_y_FSR1 = poly2(x,result.params)
diff_1 = np.max(fitted_y_FSR1) - np.min(fitted_y_FSR1)
print(diff_1)
#%% Fit second curve
params = Parameters()
params.add('b', value = 1)
params.add('c', value = 1)
x = np.arange(0,len(FSRs2[0]),1)
minner = Minimizer(fcn2min,params, fcn_args = (np.arange(0,len(FSRs2[0]),1),FSRs2[0]))
result = minner.minimize()
report_fit(result)
report = fit_report(result)
fitted_y_FSR2 = poly2(x,result.params)
diff_2 = np.max(fitted_y_FSR2) - np.min(fitted_y_FSR2)
print(diff_2)
#%%
#Plot dispersion curves
def disp_plot(FSRs, fitted,n):
    dispersion_x = range(len(FSRs[0])) #FSR number
    fig, ax4 = plt.subplots(2,1,sharex = True)
    ax4[0].set_title("C-Band dispersion curve, Sweep " + str(n))
    ax4[0].plot(dispersion_x, FSRs[0],'-', linewidth = 0.5, label =  "FSRs")
    ax4[0].plot(dispersion_x, FSRs[0],'o', markersize = 2, label =  "FSRs")
    ax4[0].set_ylabel("FSR (Hz)")
    ax4[0].set_xlabel("FSR number")
    ax4[1].set_title("Laser scan speed")
    ax4[1].plot(dispersion_x, FSRs[1],'-', linewidth = 0.5, label =  "FSRs")
    ax4[1].plot(dispersion_x, FSRs[1],'o', markersize = 2, label = "estimated laser scan speed")
    ax4[1].set_ylabel("Laser scan speed (Hz/sample)")
    ax4[1].set_xlabel("FSR number")
    
    try:
        ax4[0].plot(dispersion_x, fitted,'-', markersize = 2)
    except:
        print("No fitted value")
disp_plot(FSRs1, fitted_y_FSR1,1)
disp_plot(FSRs2, fitted_y_FSR2, 2)
#%% Fitting to dispersion curve - à faire dimanche
...
#%% Compare with other dispersion curve
# #%%
